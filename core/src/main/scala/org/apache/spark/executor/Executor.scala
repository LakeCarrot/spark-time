...
private[spark] class Executor(
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      execBackend.statusUpdate(taskId, TaskState.RUNNING, EMPTY_BYTE_BUFFER,0,0,0)
	...
	var dataSize:Long = 0
	  ...
	  m.inputMetrics match {
            case Some(metrics) =>
              dataSize = metrics.bytesRead
            case None => logError("[Rui] Wait!")
          }

          m.shuffleReadMetrics match{
            case Some(metrics) =>
              fetchSize = metrics.totalBytesRead
            case None => logError("[Rui] why?")
          }
	...
	time = taskFinish - taskStart - task.executorDeserializeTime
        ...
        execBackend.statusUpdate(taskId, TaskState.FINISHED, serializedResult, time, dataSize, fetchSize)

      } catch {
        case ffe: FetchFailedException =>
          val reason = ffe.toTaskEndReason
          execBackend.statusUpdate(taskId, TaskState.FAILED, ser.serialize(reason),0,0,0)

        case _: TaskKilledException | _: InterruptedException if task.killed =>
          logInfo(s"Executor killed $taskName (TID $taskId)")
          execBackend.statusUpdate(taskId, TaskState.KILLED, ser.serialize(TaskKilled),0,0,0)
        ...
        case t: Throwable =>
          ...
          execBackend.statusUpdate(taskId, TaskState.FAILED, ser.serialize(taskEndReason),0,0,0)
