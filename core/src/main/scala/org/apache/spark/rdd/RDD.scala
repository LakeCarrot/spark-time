...
abstract class RDD[T: ClassTag](
  ...
  def sample(
    ...
    if (withReplacement) {
      logError("[RuiSampleError] with Replacement. fraction: " + fraction + ", seed: " + seed)
  ...
  def takeSample(
    ...
    logError("[Rui6] initialCount2: " + initialCount)
    if (initialCount == 0) {
      return new Array[T](0)
    }
    ...
    logError("[Rui6] finish running sample, samples:" + samples)
    for (elem <- samples)
    {
      logError("[Rui6] elem: " + elem)
    }
