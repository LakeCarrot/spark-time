...
class DAGScheduler(
  ...
  private def submitStage(stage: Stage) {
    logError("[RuiResourceOffer] submitStage: " + stage)
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          logError("[RuiMultiCore] Submitting " + stage + " (" + stage.rdd + "), which has no missing parents")
          submitMissingTasks(stage, jobId.get)
        } else {
          for (parent <- missing) {
            logError("[Rui] submitStage. need to launch " + parent + " first before " + stage)
            submitStage(parent)
          }
          waitingStages += stage
        }
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    logError("[RuiDebug] execute stage " + stage + ", and its rdd is: " + stage.rdd)
        ...
        task match {
          ...
          case smt: ShuffleMapTask =>
            ...
            if (failedEpoch.contains(execId) && smt.epoch <= failedEpoch(execId)) {
              ...
              logError("[RuiMultiCore] task " + smt.partitionId + " has finished, and its status is: " + status)
            ...
            if (runningStages.contains(shuffleStage) && shuffleStage.pendingTasks.isEmpty) {
              ...
              logError("[RuiDebug] waitingStages: " + waitingStages)
              ...
              logError("[RuiShuffleWrite] ShuffleMapStage output: ")
              for (elem <- shuffleStage.outputLocs)
              {
                logError("[RuiShuffleWrite] elem: " + elem)
              }
              ...
              if (shuffleStage.outputLocs.contains(Nil)) {
                ...
                for (shuffleStage <- waitingStages) {
                  logError("[RuiDebug] Missing parents for " + shuffleStage + ": " + getMissingParentStages(shuffleStage))
                }
                ...
                for {
                  ...
                  logError("[RuiDebug] Submitting " + shuffleStage + " (" +
